...
Notch3 combines a native macOS desktop shell (SwiftUI & AppKit) with a local TypeScript/Node.js agent runtime built on [`ethers`](https://docs.ethers.org). The Swift module and executable retain their `NotchAgent` identifiers for compatibility.
...
- 🧭 **Guided Setup & Chat Provider**: Choose an existing User Wallet or create a new 12-word recovery phrase with explicit backup confirmation, then configure an OpenAI-compatible HTTPS/loopback provider. API keys are write-only from the UI and stored in a dedicated macOS Keychain record.
- 🔐 **Dual-Wallet Security Boundary**:
  ...
  - **Agent Wallet**: A separate wallet created during setup and funded by the user. It authorizes and settles x402 payment challenges only inside explicit user chat tasks, without a per-transaction UI prompt or nominal spend cap.
...
- 🛑 **System Lifecycle & Kill Switch**: Clears the internal agent session and purges volatile keys on screen lock (`com.apple.screenIsLocked`), app quit, or the emergency kill switch. The collapsed HUD remains branded `Notch3`; the next expansion authenticates again.
...
```bash
# Run Vitest test suites (deterministic — no live chain calls)
pnpm test
