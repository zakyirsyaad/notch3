...
public enum BIP39 {
    ...
    public static func validateMnemonic(_ mnemonic: String) -> Bool {
        ...
        guard [12, 15, 18, 21, 24].contains(count) else {
            return false
        }
    ...
    /// Derives a 64-byte seed from a BIP-39 mnemonic and optional passphrase.
    public static func seed(from mnemonic: String, passphrase: String = "") -> Data {
        let normalizedMnemonic = mnemonic.trimmingCharacters(in: .whitespacesAndNewlines)
            ...
            .filter { !$0.isEmpty }
            .joined(separator: " ")
            .decomposedStringWithCanonicalMapping // NFKD normalization

        let salt = ("mnemonic" + passphrase).decomposedStringWithCanonicalMapping
        let passwordData = normalizedMnemonic.data(using: .utf8)!
        let saltData = salt.data(using: .utf8)!

        var derivedKey = [UInt8](repeating: 0, count: 64)

        _ = passwordData.withUnsafeBytes { passPtr in
    ...
    public static let englishWordList: Set<String> = [
